            ...
            for (int j = 0; j < mapsize; j++)
            {
                if(grid[i, j]==0 && nearcubenum(i,j)==0) // An empty space with no other items within one grid of it
                {
                    if (numboss < 3) // Determine the number of bosses
                    {
                        ...
                        grid[i, j] = 3;
                        i += Random.Range(2, 6); // Make sure the boss and the gold are not too close together
                        j += Random.Range(2, 6); // Make sure the boss and the gold are not too close together
